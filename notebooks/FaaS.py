...
def get_predicted_batches(sales_clusters_df,
    ...
    for cluster in clusters_list:
        local_optimal_batches, local_predicted_batches, local_preds = get_cluster_level_predicted_batches(
            sales_clusters_df,
            cluster,
            max_waiting_time,
            test_date,
    ...
    for (cluster, dark_color, pastel_color) in zip(clusters_list, dark_map, pastel_map):
        ...
        fig.add_trace(go.Bar(x=pd.to_datetime(local_optimal[(local_optimal['batch_date'] > test_date) & \
                                                            (local_optimal['batch_date'] < str((pd.Timestamp(
                                                                test_date) + pd.Timedelta(calendar_length))))] \
                                                  ['batch_date']) - pd.Timedelta('0 hours'),
                             y=local_optimal[(local_optimal['batch_date'] > test_date) & \
                                             (local_optimal['batch_date'] < str(
                                                 (pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))] \
                                 ['quantities'],
                             name='Cluster #{}\nOptimized batches - actual values'.format(cluster),
                             width=1e3 * pd.Timedelta('6 hours').total_seconds(),
                             marker_color=dark_color,
    ...
    for (cluster, dark_color, pastel_color) in zip(clusters_list, dark_map, pastel_map):
        ...
        if local_preds[(local_preds.ds > test_date) & (
                local_preds.ds <= str((pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))].shape[0] > 0:
            display(HTML(local_preds[(local_preds.ds > test_date) & (
                    local_preds.ds <= str((pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))][
                             ['ds', 'y', 'product', 'Cluster']].to_html()))

        if local_preds[(local_preds.yhat_date > test_date) & (
                local_preds.yhat_date <= str((pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))].shape[0] > 0:
            display(HTML(local_preds[(local_preds.yhat_date > test_date) & (
                    local_preds.yhat_date <= str((pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))][
                             ['yhat_date', 'yhat_qty', 'product', 'Cluster']].to_html()))
        ...
        fig.add_trace(go.Scatter(x=pd.to_datetime(local_preds[(local_preds.ds > test_date) & (
                local_preds.ds <= str((pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))]['ds']),
                                 y=local_preds[(local_preds.ds > test_date) & (local_preds.ds <= str(
                                     (pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))]['y'],
                                 marker=dict(
                                     color=dark_color,
                                     size=10,
                                     line=dict(
        ...
        fig.add_trace(go.Scatter(x=pd.to_datetime(local_preds[(local_preds.yhat_date > test_date) & (
                local_preds.yhat_date <= str((pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))][
                                                      'yhat_date']),
                                 y=local_preds[(local_preds.yhat_date > test_date) & (local_preds.yhat_date <= str(
                                     (pd.Timestamp(test_date) + pd.Timedelta(calendar_length))))]['yhat_qty'],
                                 marker=dict(
                                     color=pastel_color,
                                     size=10,
                                     line=dict(
...
def optimize_orders_processing(sales_clusters_df, cluster, maximum_waiting_time, detailed_view):
    sales_cluster = sales_clusters_df[sales_clusters_df['Cluster'] == cluster][
        ['noisy_date', 'noisy_quantity', 'product']]. \
    ...
    batch_dates_df['batch'] = np.max(
        sales_clusters_df[sales_clusters_df['Cluster'] == cluster]['noisy_quantity'].values)

    if detailed_view:
        fig = go.Figure()

        fig.add_trace(
            go.Scatter(x=sales_clusters_df[sales_clusters_df['Cluster'] == cluster]['noisy_date'].astype(str).values,
        ...
        fig.update_yaxes(
            range=[np.min(sales_clusters_df[sales_clusters_df['Cluster'] == cluster]['noisy_quantity'].values),
...
def generate_models_n_preds_df_n_plotly_viz(sales_clusters_df, test_start_date, product, detailed_view):
    ...
    # Renaming the prophet output columns to identify the quantity preds (in comparison with the sales dates preds)
...
def get_aggregated_preds(sales_clusters_df, cluster, test_start_date, detailed_view):
    ...
    for prod in cluster_products:
        cluster_preds.append(
            generate_models_n_preds_df_n_plotly_viz(sales_clusters_df, test_start_date, prod, detailed_view))
...
def get_cluster_level_predicted_batches(sales_clusters_df,
    ...
    sales_cluster_optimized_preds = optimize_orders_processing(cluster_aggregated_preds, cluster, max_waiting_time,
                                                               detailed_view)
    ...
    if detailed_view:
        ...
        try:
            g = sns.jointplot(
                pd.Series(full_preds['error_days'].values / (24 * 60 * 60 * 1e9), name='error_days\nin days'),
